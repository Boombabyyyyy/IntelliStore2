...
<template>
    ...
    <div v-if="isadmin == 1" class="site-section">
      <div class="container">
      <div class="row">
        <div class="col-md-12">
    <h2 class="h3 mb-3 text-black">Manager access Approval</h2>
    <div class="col-md-18">
        <div class="col-md-18">
          <table class="table table-bordered">
            ...
            <tbody>
              <tr v-for="approval in mgrapprovals" :key="approval.user_id">
                ...
                <td class="product-name">
                  <button @click="handleMgrApproval(approval.user_id, 0)" class="btn btn-danger btn-sm" value="" style="float:right;"><span class="icon icon-close"> </span></button>
                <button @click="handleMgrApproval(approval.user_id, 1)" class="btn btn-success btn-sm" value="" style="float:right; margin-right:10px;"><span class="icon icon-check"> </span></button>
                </td>
              </tr>
            </tbody>
            </table>
          ...
          <table class="table table-bordered">
            ...
            <tbody>
              <tr v-for="approval in catapprovals" :key="approval.approval_id">
                ...
                <td v-if="approval.status == 'pending'" class="product-name">
                  <button @click="handleCatApproval(approval.approval_id, 0)" class="btn btn-danger btn-sm" value="" style="float:right;"><span class="icon icon-close"> </span></button>
                <button @click="handleCatApproval(approval.approval_id, 1)" class="btn btn-success btn-sm" value="" style="float:right; margin-right:10px;"><span class="icon icon-check"> </span></button>
                </td>
              </tr>
            </tbody>
            </table>
...
export default {
  ...
  methods: {
    async handleCatApproval(approval_id, approval){
      const response = await fetchUtil({
          endpoint: "catapprovals",
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          data: {"approval_id": approval_id, "approve": approval},
          user_id: this.user_id
        });
        console.log(response);
        location.reload();
    },
    async handleMgrApproval(user_id, approval){
      const response = await fetchUtil({
          endpoint: "mgr",
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          data: {"user_id": user_id, "approve": approval},
          user_id: this.user_id
        });
        console.log(response);
        location.reload();
    ...
    confirmDeletecat(cat_id, name){
      const isConfirmed = window.confirm(`Are you sure you want to delete the ${name} category?`);
